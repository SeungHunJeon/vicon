...
#define RSMSG(msg, severity) { std::stringstream raimessagestream; \
                                raimessagestream<<msg; \
                                  RaiSimMsg().stream(__FILE__, __LINE__, raimessagestream, severity); }

#define RSINFO(msg) RSMSG(msg, RSEVERITY_INFO)
#define RSWARN(msg) RSMSG(msg, RSEVERITY_WARN)
#define RSFATAL(msg) RSMSG(msg, RSEVERITY_FATAL)
#define RSRETURN(con, msg) RSMSG(msg, raisim::RSEVERITY_INFO)return;

#define RSINFO_IF(con, msg) if(con) RSMSG(msg, RSEVERITY_INFO)
#define RSWARN_IF(con, msg) if(con) RSMSG(msg, RSEVERITY_WARN)
#define RSFATAL_IF(con, msg) if(con) RSMSG(msg, RSEVERITY_FATAL)
#define RSASSERT(con, msg) if(!(con)) RSMSG(msg, RSEVERITY_FATAL)
